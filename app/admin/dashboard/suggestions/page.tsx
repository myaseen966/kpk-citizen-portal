...
export default function AdminSuggestionsPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/applications')
      .then((r) => r.json())
      .then((data) => { setSuggestions(data.suggestions || []); setIsLoading(false) })
      .catch(console.error)
  }, [])
  ...
  async function handleUpdateStatus() {
    ...
    try {
      const res = await fetch('/api/admin/applications', {
        ...
        body: JSON.stringify({ id: selected.id, type: 'suggestion', status: newStatus, adminNote }),
      })
      if (res.ok) {
        setSuccess('Status updated and citizen notified!')
        setSuggestions((prev) =>
          prev.map((s) => s.id === selected.id ? { ...s, status: newStatus, adminNote } : s)
        )
        setSelected(null)
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#273F5B' }}>Suggestions</h1>
          <p className="text-sm" style={{ color: '#6A85A6' }}>Manage all citizen suggestions</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ border: '1px solid #839DBC' }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8" style={{ color: '#6A85A6' }}>No suggestions found.</td></tr>
            ) : (
              filtered.map((s) => (
                <tr key={s.id} style={{ borderBottom: '1px solid #f0f4f8' }}>
                  <td className="px-4 py-3">
                    {s.hideIdentity ? <span style={{ color: '#6A85A6' }}>Hidden</span> : (
                      <div>
                        <div className="font-medium" style={{ color: '#273F5B' }}>{s.citizen.fullName}</div>
                        <div className="text-xs" style={{ color: '#6A85A6' }}>{s.citizen.cnic}</div>
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3" style={{ color: '#273F5B' }}>{s.subject}</td>
                  <td className="px-4 py-3" style={{ color: '#6A85A6' }}>{s.category}</td>
                  <td className="px-4 py-3" style={{ color: '#6A85A6' }}>{new Date(s.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-1 rounded-full font-medium" style={statusStyle(s.status)}>{s.status}</span>
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => { setSelected(s); setNewStatus(s.status); setAdminNote(s.adminNote || '') }} className="text-xs font-semibold px-3 py-1.5 rounded-lg text-white" style={{ backgroundColor: '#0083FF' }}>Manage</button>
                  </td>
                </tr>
              ))
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(39,63,91,0.7)' }}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-lg" style={{ color: '#273F5B' }}>Manage Suggestion</h2>
            ...
            <div className="rounded-xl p-4 mb-4 space-y-2" style={{ backgroundColor: '#f0f4f8' }}>
              ...
              <div><span className="text-xs font-medium" style={{ color: '#6A85A6' }}>Department: </span><span className="text-sm" style={{ color: '#273F5B' }}>{selected.department}</span></div>
